...
| `user_id` | BIGINT | foreign_id_constrained | **FK** → `users.id`, NOT NULL, INDEXED | The user who created and published the video. |
| `video_id` | BIGINT | foreign_id_constrained | **FK** → `videos.id`, UNIQUE, NOT NULL, INDEXED | The processed video being published. A video can only be in the gallery once. |
...
| `name` | STRING | string | 🔒 PII, 🔐 ENCRYPTED | The user's display name. |
| `email` | STRING | string | UNIQUE, NOT NULL, INDEXED, 🔒 PII, 🔐 ENCRYPTED | The user's unique email address, used for login and communication. |
...
- **belongsToMany()**: Many-to-Many (belongsToMany) → `User`
...
- **belongsToMany()**: Many-to-Many (belongsToMany) → `Effect`
...
- **hasMany()**: One-to-Many (hasMany/belongsTo) → `Subscription`
...
| `category_id` | BIGINT | foreign_id_constrained | **FK** → `categories.id`, INDEXED | The category this filter belongs to. |
...
- **belongsTo()**: One-to-Many (hasMany/belongsTo) → `Category`
...
| `user_id` | BIGINT | foreign_id_constrained | **FK** → `users.id`, NOT NULL, INDEXED | Foreign key linking to the user who owns the video. |
| `effect_id` | BIGINT | foreign_id_constrained | **FK** → `effects.id`, NOT NULL, INDEXED | Foreign key linking to the AI effect applied to the video. |
| `original_file_id` | BIGINT | foreign_id_constrained | **FK** → `files.id`, NOT NULL, INDEXED | Foreign key linking to the original video file uploaded by the user. |
| `processed_file_id` | BIGINT | foreign_id_constrained | **FK** → `files.id`, INDEXED | Foreign key linking to the processed video file after the AI effect was applied. |
...
- **belongsTo()**: One-to-Many (hasMany/belongsTo) → `User`
- **belongsTo()**: One-to-Many (hasMany/belongsTo) → `Effect`
- **belongsTo()**: One-to-One (hasOne/belongsTo) → `File`
- **belongsTo()**: One-to-One (hasOne/belongsTo) → `File`
- **hasMany()**: One-to-Many (hasMany/belongsTo) → `Export`
...
- **hasMany()**: One-to-Many (hasMany/belongsTo) → `Effect`
...
| `user_id` | BIGINT | foreign_id_constrained | **FK** → `users.id`, NOT NULL, INDEXED | Foreign key linking to the user who initiated the export. |
| `video_id` | BIGINT | foreign_id_constrained | **FK** → `videos.id`, NOT NULL, INDEXED | Foreign key linking to the video being exported. |
...
| `exported_file_id` | BIGINT | foreign_id_constrained | **FK** → `files.id`, INDEXED | Foreign key linking to the final exported file, available for download. |
...
- **belongsTo()**: One-to-Many (hasMany/belongsTo) → `User`
- **belongsTo()**: One-to-Many (hasMany/belongsTo) → `Video`
- **belongsTo()**: One-to-One (hasOne/belongsTo) → `File`
...
| `effect_id` | BIGINT | foreign_id_constrained | **FK** → `effects.id`, INDEXED | Optional foreign key if this style is specific to one effect. |
...
| `category_id` | BIGINT | foreign_id_constrained | **FK** → `categories.id`, INDEXED | Foreign key linking to the effect's category for organization. |
| `model_id` | BIGINT | foreign_id_constrained | **FK** → `models.id`, INDEXED | Foreign key linking to the underlying AI model that powers this effect. |
...
- **belongsTo()**: One-to-Many (hasMany/belongsTo) → `Category`
- **belongsTo()**: One-to-Many (hasMany/belongsTo) → `AiModel`
- **hasMany()**: One-to-Many (hasMany/belongsTo) → `Style`
- **hasMany()**: One-to-Many (hasMany/belongsTo) → `Video`
...
| `tier_id` | BIGINT | foreign_id_constrained | **FK** → `tiers.id`, INDEXED | Foreign key to the tiers table, if this package grants a subscription tier. |
...
- **belongsTo()**: One-to-Many (hasMany/belongsTo) → `Tier`
...
| `purchase_id` | BIGINT | foreign_id_constrained | **FK** → `purchases.id`, INDEXED | The purchase where this reward was applied. |
| `created_at` | TIMESTAMP | timestamp | NOT NULL | Auto-managed |
| `updated_at` | TIMESTAMP | timestamp | NOT NULL | Auto-managed |
...
- **morphedByMany()**: Polymorphic (morphTo/morphMany) → `Overlay`
- **morphedByMany()**: Polymorphic (morphTo/morphMany) → `Watermark`
...
| `user_id` | BIGINT | foreign_id_constrained | **FK** → `users.id`, NOT NULL, INDEXED | The user who owns this subscription. |
| `package_id` | BIGINT | foreign_id_constrained | **FK** → `packages.id`, NOT NULL, INDEXED | The package this subscription is for. |
| `tier_id` | BIGINT | foreign_id_constrained | **FK** → `tiers.id`, NOT NULL, INDEXED | The feature tier granted by this subscription. |
...
- **belongsTo()**: One-to-Many (hasMany/belongsTo) → `User`
- **belongsTo()**: One-to-Many (hasMany/belongsTo) → `Package`
- **belongsTo()**: One-to-Many (hasMany/belongsTo) → `Tier`
...
| `file_id` | BIGINT | foreign_id_constrained | **FK** → `files.id`, NOT NULL, INDEXED | Reference to the file containing the overlay asset (e.g., SVG, Lottie JSON). |
...
- **belongsTo()**: One-to-One (hasOne/belongsTo) → `File`
- **morphToMany()**: Polymorphic (morphTo/morphMany) → `Tag`
...
| `user_id` | BIGINT | foreign_id_constrained | **FK** → `users.id`, NOT NULL, INDEXED | The user who owns this file. |
...
| `original_filename` | STRING | string | 🔒 PII, 🔐 ENCRYPTED | The original filename as uploaded by the user. Can contain PII. |
...
- **hasOne()**: One-to-One (hasOne/belongsTo) → `Overlay`
- **belongsTo()**: One-to-Many (hasMany/belongsTo) → `User`
...
- **CreditTransaction**::belongsTo() → **User** (through ``)  
  FK: `user_id`

- **User**::hasMany() → **CreditTransaction** (through ``)  
  FK: `user_id`  
  ⚠️ CASCADE DELETE
...
- **User**::hasMany() → **GalleryVideo** (through ``)  
  FK: `user_id`  
  ⚠️ CASCADE DELETE

- **Purchase**::belongsTo() → **User** (through ``)  
  FK: `user_id`
...
- **Video**::belongsTo() → **User** (through `null`)  
  FK: `user_id`

- **Video**::belongsTo() → **Effect** (through `null`)  
  FK: `effect_id`

- **Video**::hasMany() → **Export** (through `null`)  
  FK: `video_id`  
  ⚠️ CASCADE DELETE

- **AiModel**::hasMany() → **Effect** (through `null`)  
  FK: `model_id`

- **Export**::belongsTo() → **User** (through `null`)  
  FK: `user_id`

- **Export**::belongsTo() → **Video** (through `null`)  
  FK: `video_id`

- **Style**::belongsTo() → **Effect** (through `null`)  
  FK: `effect_id`

- **Effect**::belongsTo() → **Category** (through `null`)  
  FK: `category_id`

- **Effect**::belongsTo() → **AiModel** (through `null`)  
  FK: `model_id`

- **Effect**::hasMany() → **Style** (through `null`)  
  FK: `effect_id`  
  ⚠️ CASCADE DELETE

- **Effect**::hasMany() → **Video** (through `null`)  
  FK: `effect_id`

- **Package**::belongsTo() → **Tier** (through `null`)  
  FK: `tier_id`

- **Reward**::belongsTo() → **User** (through `null`)  
  FK: `user_id`

- **Subscription**::belongsTo() → **User** (through `null`)  
  FK: `user_id`

- **Subscription**::belongsTo() → **Package** (through `null`)  
  FK: `package_id`

- **Subscription**::belongsTo() → **Tier** (through `null`)  
  FK: `tier_id`

- **File**::belongsTo() → **User** (through `null`)  
  FK: `user_id`

### Polymorphic (morphTo/morphMany)

- **Overlay**::morphToMany() → **Tag** (through `taggables`)  
  FK: `tag_id`  
  ⚠️ CASCADE DELETE

- **Watermark**::morphToMany() → **Tag** (through `taggables`)  
  FK: `tag_id`  
  ⚠️ CASCADE DELETE

- **Tag**::morphedByMany() → **Overlay** (through `taggables`)  
  FK: `taggable_id`

- **Tag**::morphedByMany() → **Watermark** (through `taggables`)  
  FK: `taggable_id`

### Many-to-Many (belongsToMany)

- **User**::belongsToMany() → **Discount** (through `discount_user`)  
  FK: `discount_id`

- **Discount**::belongsToMany() → **User** (through `discount_user`)  
  FK: `user_id`

- **Category**::belongsToMany() → **Effect** (through `category_effect`)  
  FK: `effect_id`

- **Algorithm**::belongsToMany() → **Effect** (through `algorithm_effect`)  
  FK: `effect_id`
...
- **Purchase**::hasOne() → **CreditTransaction** (through ``)  
  FK: `related_purchase_id`  
  ⚠️ CASCADE DELETE
...
- **Export**::belongsTo() → **File** (through `null`)  
  FK: `exported_file_id`  
  ⚠️ CASCADE DELETE

- **Reward**::belongsTo() → **Purchase** (through `null`)  
  FK: `purchase_id`

- **Overlay**::belongsTo() → **File** (through `null`)  
  FK: `file_id`

- **File**::hasOne() → **Overlay** (through `null`)  
  FK: `file_id`  
  ⚠️ CASCADE DELETE
