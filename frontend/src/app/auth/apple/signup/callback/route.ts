...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    res = await fetch(`${backendBase}/auth/apple/signup/callback`, {
      method: "POST",
    ...
    return NextResponse.redirect(
      `${origin}/auth/apple/signup/done?error=${encodeURIComponent(msg)}`,
    );
  }
  ...
  return NextResponse.redirect(
    `${origin}/auth/apple/signup/done?error=${encodeURIComponent(errorMsg)}`,
  );
}
