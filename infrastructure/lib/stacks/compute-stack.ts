...
export class ComputeStack extends cdk.Stack {
  ...
  private setupServicesAndTargetGroups(
    ...
    const buildPhpBootstrapCommand = (processCommand: string): string => [
      ...
      `  tmp="${applePrivateKeyPath}.tmp"`,
      `  if printf '%s' "$APPLE_PRIVATE_KEY_P8_B64" | base64 -d > "$tmp" 2>/dev/null; then`,
      `    mv "$tmp" "${applePrivateKeyPath}"`,
      '  else',
      '    echo "APPLE_PRIVATE_KEY_P8_B64 is not valid base64; skipping Apple key write." >&2',
      '    rm -f "$tmp" || true',
      '  fi',
      'fi',
      `exec ${processCommand}`,
